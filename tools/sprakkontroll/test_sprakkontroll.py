...
def granska_med(stad_funktion, namn, text, sprak):
    ...
    try:
        return cl.granska(namn, text, sprak, REGLER, EGENNAMN)
    finally:
        cl.stad = original
...
def test_2_publicerad_norsk_sida():
    print("\nTEST 2 — verklig publicerad no/index.html tomdes helt (fynd 1)")
    sokvag = os.path.join(ROOT, "no", "index.html")
    if not os.path.exists(sokvag):
        kolla(False, "no/index.html saknas i repot — testet kunde inte koras")
        return
    raw = open(sokvag, encoding="utf-8", errors="replace").read()
